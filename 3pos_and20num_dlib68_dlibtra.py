...
def findPeople(features_arr, positions, thres = 0.7, percent_thres = 70):
    ...
    '''
    f = open('./facerec_128D_dlib68.txt','r')
...
def track(frame,face_box,recog_data):
	...
	#print (type(rect))
	tracker[i].start_track(frame, dlib.rectangle(rect.left(),rect.top(),rect.right(),rect.bottom()))
	#print (rect)
	...
	for i in xrange(len(tracker)):
		...
		cv2.putText(img,"Only Track FPS:" + str(fps),(5,30),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),1,cv2.LINE_AA)	
